...
public class MapGeneratorController : MonoBehaviour
{
    ...
    [Header("Seed")] public bool randomizeSeed = false;
    ...
    public void GenerateMap(bool randomizeSeed = false)
    {
        if (randomizeSeed)
            RandimizeSeed();
    ...
    public void DebugGridMesh()
    {
        ...
        foreach (var gridCell in MainInfoGrid.GetGridArray())
        {
            var cell = gridCell;
            if (cell == null) continue;
            if (MainGridData.materials.TryGetValue(cell.GridCellType, out var material))
            {
                var createdCell = new GameObject($"cell nr x:{cell.Coordinate.x} y:{cell.Coordinate.y}");
                createdCell.transform.parent = transform;

                createdCell.transform.position = cell.Position + new Vector3(MainGridData.cellScale, 0, MainGridData.cellScale) * 0.5f;

                createdCell.transform.localScale = new Vector3(MainGridData.cellScale, MainGridData.cellScale, MainGridData.cellScale);
