...
public struct RoomGanerateSetting
{
    ...
    {
        // Inicjalizacja generatora z zadanym seedem
        Random random = new Unity.Mathematics.Random(seed);
        ...
        for (int i = 0; i < RoomCount; i++)
        {
            ...
            // Próbuj wygenerować pokój maksymalnie MaxAttempts razy
            while (attempts < MaxAttempts && room == null)
            {
                // Przekazujemy random przez referencję
                room = GenerateRoom(ref random, generatedGrid);
                attempts++;
            }
            ...
            // Dodaj wygenerowany pokój do listy
            createdRoom.Add(room);
        }

        foreach (var room in createdRoom)
        {
            room.MarkBorders(createdRoom);
            room.MarkCorners(createdRoom);
        }
    }

    private Room GenerateRoom(ref Random random, Grid<GridCellData> gridData)
    {
        Room room = new Room();
        room.CellInRoom = new List<GridCellData>();

        // Wybór losowych wymiarów pokoju
        ...
        // Wybór losowej pozycji, uwzględniając rozmiar pokoju
        int x = random.NextInt(0, gridData.GeneratedGridSize().x - width);
        int y = random.NextInt(0, gridData.GeneratedGridSize().y - height);

        // Sprawdzenie warunków przed umieszczeniem pokoju
        if (!CanPlaceRoom() || !IsWithinMaxDistanceOfAnyRoom())
        {
            return null;
        }

        // Przypisanie komórek do pokoju
    ...
    private bool CanPlaceRoom()
    {
        /*for (int x = start.x - MinRoomDistance; x < start.x + width + MinRoomDistance; x++)
        {
            for (int y = start.y - MinRoomDistance; y < start.y + height + MinRoomDistance; y++)
            {
                ...
                if (cell != null && cell.GridCellType == E_GridCellType.RoomCorner ||
    ...
    private bool IsWithinMaxDistanceOfAnyRoom()
    {
        return true;
